...
const (
	...
	INBOUND_ID = 16 // Инбаунд 16 (обновлено)
)

func main() {
	log.Println("=== ТЕСТОВЫЙ СКРИПТ СОЗДАНИЯ ПОДПИСКИ С ПРЕФИКСОМ _1 ===")
	...
	// Настраиваем webhook для получения обновлений
...
func handleMessage(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	log.Printf("Получено сообщение от %d: %s", message.From.ID, message.Text)

	// Проверяем команду /start
	if message.IsCommand() && message.Command() == "start" {
		log.Printf("Обработка команды /start для пользователя %d", message.From.ID)
		...
		if err != nil {
			log.Printf("Ошибка работы с пользователем: %v", err)
			sendError(bot, message.Chat.ID, "Ошибка работы с базой данных")
			return
		}
		...
		if err := createSubscriptionWithPrefix(bot, user, message.Chat.ID); err != nil {
			log.Printf("Ошибка создания подписки: %v", err)
			sendError(bot, message.Chat.ID, "Ошибка создания подписки: "+err.Error())
			return
		}
		...
		msg := tgbotapi.NewMessage(message.Chat.ID,
			"✅ Подписка успешно создана!\n\n"+
				"🔗 Ваш конфиг готов к использованию\n"+
				"📱 Скачайте приложение и импортируйте конфигурацию\n\n"+
				"💡 Подписка создана с префиксом _1")
		msg.ParseMode = "HTML"
...
func createSubscriptionWithPrefix(bot *tgbotapi.BotAPI, user *common.User, chatID int64) error {
	...
	log.Printf("   🌊 Flow: %s", newClient.Flow)
		...
		if err := json.Unmarshal([]byte(updatedInbound.Settings), &updatedSettings); err != nil {
			...
			for _, client := range updatedSettings.Clients {
				if client.Email == email {
					log.Printf("✅ Клиент найден в панели:")
	...
	configURL := fmt.Sprintf("https://shadowfade.ru:6187/E4x7DnWKY8QnRdDoc3/%s", subID)

	msg := tgbotapi.NewMessage(chatID,
		fmt.Sprintf("🎉 <b>Подписка создана успешно!</b>\n\n"+
			"📧 Email: <code>%s</code>\n"+
			"🔑 SubID: <code>%s</code>\n"+
			"⏰ Действует до: %s\n\n"+
			"🔗 <a href=\"%s\">Скачать конфигурацию</a>\n\n"+
			"💡 <i>Подписка создана с префиксом _1 как требовалось</i>",
			email, subID,
			time.UnixMilli(expiryTime).Format("02.01.2006 15:04"),
			configURL))
	msg.ParseMode = "HTML"

	if _, err := bot.Send(msg); err != nil {
		log.Printf("Ошибка отправки сообщения с конфигурацией: %v", err)
	}
