...
func HandleCallback(bot *tgbotapi.BotAPI, callback *tgbotapi.CallbackQuery) {
	// Логируем callback пользователя
	common.LogUserCallback(callback)
	...
	switch {
	case data == "balance":
		// Логируем нажатие кнопки баланса
		username := ""
		if callback.From.UserName != "" {
			username = callback.From.UserName
		}
		common.LogUserButtonClick(userID, username, callback.From.FirstName, callback.From.LastName, "Balance", data)

		log.Printf("HANDLE_CALLBACK: Вызов editBalance для TelegramID=%d", userID)
		menus.EditBalance(bot, chatID, messageID, user)
	case data == "vpn":
		// Логируем нажатие кнопки VPN
		username := ""
		if callback.From.UserName != "" {
			username = callback.From.UserName
		}
		common.LogUserButtonClick(userID, username, callback.From.FirstName, callback.From.LastName, "VPN", data)
...
func handlePayCallback(bot *tgbotapi.BotAPI, chatID int64, messageID int, user *common.User, data string, callback *tgbotapi.CallbackQuery) {
	...
	// Логируем нажатие кнопки оплаты
	common.LogUserButtonClick(userID, username, callback.From.FirstName, callback.From.LastName,
		fmt.Sprintf("Pay %d days", days), data)
