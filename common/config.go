...
func init() {
	...
	CONFIG_BASE_URL = "https://status.moment-was-da.ru:5913/sH6pDf4KTvHRDequWW/"
	CONFIG_JSON_URL = "https://status.moment-was-da.ru:5913/4jN9aoc8upMptvTRk/"

	REDIRECT_DOMAIN = "status.moment-was-da.ru:8081" // редирект для импорта подписки
	REDIRECT_IMPORT = "happ"                         // тип импорта: "happ" или "v2raytun"

	// ---TRAFIC---
	// TRAFFIC_RESET_ENABLED = true   // включен ли автоматический сброс трафика
	// PRICE_PER_DAY = 8              // стоимость подписки за день
	...
	TRAFFIC_RESET_ENABLED = true // включен ли автоматический сброс трафика
	PRICE_PER_DAY = 8            // стоимость подписки за день
	TRAFFIC_LIMIT_GB = 3         // лимиты трафика. Срок действия указывается в TRAFFIC_RESET_INTERVAL
	TRAFFIC_RESET_INTERVAL = 30  // 7 дней в минутах (7*24*60 = 8640)
	TRAFFIC_CHECK_INTERVAL = 1   // минуты. как часто бот проверяет, не превысил ли пользователь лимит трафика
	...
	MAX_IPS_PER_CONFIG = 5                              // Максимальное количество IP адресов на один конфиг (если больше - отключается)
	...
	IP_CLEANUP_INTERVAL = 3                             // Интервал очистки старых данных в часах
	// не используется
	...
	TRIAL_BALANCE_AMOUNT = 240 // сумма в рублях, которая добавляется при активации пробного периода (минимум = PRICE_PER_DAY)

	// Ссылка на поддержку - куда направлять пользователей при нажатии на кнопку "Поддержка"
	SUPPORT_LINK = "https://t.me/moment_was"

	// ---АВТОСПИСАНИЕ---
	AUTO_BILLING_ENABLED = true // Включено ли автосписание за каждый день
	BALANCE_RECALC_INTERVAL = 1 // Интервал пересчета дней по балансу в минутах (1 час - разумный интервал)
	...
	DUPLICATE_CLEANUP_INTERVAL = 30  // Интервал очистки дубликатов в минутах (5 суток = 5*24*60 = 7200)
	...
	YUKASSA_SHOP_ID = "1164658"                                                  // ID магазина ЮКассы
	YUKASSA_SECRET_KEY = "live_Pk-4gOjwgdNO4Ip0bBVIh8diCSBp_9_ZHEpj_eq8Pu4"      // Секретный ключ ЮКассы
	YUKASSA_API_PAYMENTS_ENABLED = true                                          // Включить платежи через прямое API
	YUKASSA_WEBHOOK_URL = "https://status.moment-was-da.ru:8081/yukassa/webhook" // URL для уведомлений от ЮКассы
	...
	REFERRAL_BONUS_AMOUNT = 1000                                     // Сумма бонуса для пригласившего (в рублях)
	REFERRAL_WELCOME_BONUS = 500                                     // Сумма бонуса для приглашенного (в рублях)
	REFERRAL_LINK_BASE_URL = "https://t.me/shadowfadebot?start=ref_" // Базовый URL для реферальных ссылок
