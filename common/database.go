...
func ResetAllTraffic() error {
	...
	for i := range settings.Clients {
		...
		// Включаем клиента если он был отключен и генерируем новый UUID
		if !client.Enable {
			client.Enable = true
			newUUID := uuid.New().String()
			client.ID = newUUID
			enabledCount++
			log.Printf("RESET_ALL_TRAFFIC: Включаем клиента с новым UUID: Email=%s, NewUUID=%s", client.Email, newUUID)
		}
