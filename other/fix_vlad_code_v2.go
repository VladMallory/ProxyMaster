...
func main() {
	log.Println("=== ИСПРАВЛЕНИЕ КОДА VLAD V2 ===")
	...
	// Сначала очищаем код у Vlad
	log.Printf("Очищаем код у Vlad")
	_, err = db.Exec(`
		UPDATE users 
	...
	if err != nil {
		log.Fatalf("❌ Ошибка очистки кода: %v", err)
	}

	log.Printf("✅ Код очищен у Vlad")

	// Теперь устанавливаем правильный код (код Славы)
	correctCode := "5035512654654"
	log.Printf("Устанавливаем код Славы: %s", correctCode)
	...
	if err != nil {
		log.Fatalf("❌ Ошибка установки кода: %v", err)
	}

	log.Printf("✅ Код установлен")
