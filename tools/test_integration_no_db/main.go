...
func main() {
	log.Printf("=== ТЕСТ ИНТЕГРАЦИИ СИНХРОНИЗАЦИИ С ДОПОЛНИТЕЛЬНЫМ ИНБАУНДОМ (БЕЗ БД) ===")

	// Создаем тестового пользователя с активным основным конфигом
	testUser := &common.User{
		TelegramID:               777777777, // Новый тестовый ID
		Username:                 "integration_test_user_2",
		FirstName:                "Integration Test 2",
		Balance:                  100.0,
		HasActiveConfig:          true,
		ClientID:                 "test-main-client-id-2",
		Email:                    "777777777",
		SubID:                    "test-main-sub-id-2",
	...
	// Временно отключаем сохранение в БД для тестирования
	// Создаем копию функции SyncUserWithSecondaryPanel без вызова UpdateUser
	...
	log.Printf("\n=== РЕЗУЛЬТАТ ===")
	if testUser.HasActiveSecondaryConfig && testUser.SecondaryEmail == "777777777_1" {
...
func syncUserWithSecondaryPanelNoDB(user *common.User) error {
	...
	// Ищем клиента пользователя в дополнительном инбаунде по email с суффиксом _1
