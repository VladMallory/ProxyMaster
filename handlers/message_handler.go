...
func handleCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, user *common.User) {
	...
	common.LogUserCommand(telegramUser.ID, username, telegramUser.FirstName, telegramUser.LastName, message.Command())
	...
	case "check":
		handleCheckCommand(bot, message, user)
	case "reset_trial":
...
func ensureUserHasConfig(bot *tgbotapi.BotAPI, user *common.User, chatID int64) {
	...
	// Автоматическая очистка призрачных записей в панели при каждом /start
	log.Printf("ENSURE_CONFIG: 🧹 Автоматическая очистка призрачных записей в панели для пользователя %d", user.TelegramID)
	go func() {
		if err := common.CleanupPhantomClients(); err != nil {
			log.Printf("ENSURE_CONFIG: ⚠️ Ошибка очистки призрачных записей для пользователя %d: %v", user.TelegramID, err)
		} else {
			log.Printf("ENSURE_CONFIG: ✅ Призрачные записи очищены для пользователя %d", user.TelegramID)
		}
	}()
	...
	go func() {
		...
		if updatedUser.Balance <= 0 {
			log.Printf("ENSURE_CONFIG: ❌ У пользователя %d нет баланса (%.2f₽), пропускаем создание конфига",
				updatedUser.TelegramID, updatedUser.Balance)
			return
		}
		...
		if !common.AUTO_BILLING_ENABLED || common.TARIFF_MODE_ENABLED {
			log.Printf("ENSURE_CONFIG: ❌ Автосписание отключено (AUTO_BILLING_ENABLED=%v) или включен тарифный режим (TARIFF_MODE_ENABLED=%v), пропускаем создание конфига для пользователя %d",
				common.AUTO_BILLING_ENABLED, common.TARIFF_MODE_ENABLED, updatedUser.TelegramID)
			return
		}

		log.Printf("ENSURE_CONFIG: ❌ У пользователя %d есть баланс %.2f₽, но нет конфига. Запускаем принудительный пересчет",
			updatedUser.TelegramID, updatedUser.Balance)
		log.Printf("ENSURE_CONFIG: 🚀 Запуск ForceBalanceRecalculation для пользователя %d", updatedUser.TelegramID)
...
// handleCheckCommand обрабатывает команду /check для принудительной проверки и исправления конфигов всех клиентов
func handleCheckCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, user *common.User) {
	log.Printf("HANDLE_CHECK: ===== ОБРАБОТКА КОМАНДЫ /check =====")
	log.Printf("HANDLE_CHECK: Выполнение команды /check для TelegramID=%d", user.TelegramID)

	// Отправляем сообщение о начале проверки
	msg := tgbotapi.NewMessage(message.Chat.ID, "🔍 Начинаю проверку и исправление конфигов всех клиентов...")
	if _, err := bot.Send(msg); err != nil {
		log.Printf("HANDLE_CHECK: Ошибка отправки сообщения: %v", err)
	}

	// Получаем всех пользователей из базы данных
	log.Printf("HANDLE_CHECK: Получение списка всех пользователей")
	users, err := common.GetAllUsers()
	if err != nil {
		log.Printf("HANDLE_CHECK: Ошибка получения пользователей: %v", err)
		msg := tgbotapi.NewMessage(message.Chat.ID, "❌ Ошибка получения списка пользователей")
		bot.Send(msg)
		return
	}

	log.Printf("HANDLE_CHECK: Найдено %d пользователей для проверки", len(users))

	// Проверяем каждого пользователя
	checkedCount := 0
	fixedCount := 0

	for i, checkUser := range users {
		log.Printf("HANDLE_CHECK: [%d/%d] Проверка пользователя %d (%s)", i+1, len(users), checkUser.TelegramID, checkUser.FirstName)

		// Выполняем проверку для каждого пользователя
		ensureUserHasConfig(bot, &checkUser, message.Chat.ID)
		checkedCount++
	...
	msg = tgbotapi.NewMessage(message.Chat.ID, fmt.Sprintf("✅ Проверка завершена!\n📊 Проверено пользователей: %d\n🔧 Исправлено конфигов: %d", checkedCount, fixedCount))
	if _, err := bot.Send(msg); err != nil {
		log.Printf("HANDLE_CHECK: Ошибка отправки сообщения: %v", err)
	}

	// Показываем обновленное главное меню
	log.Printf("HANDLE_CHECK: Отправка обновленного главного меню для пользователя %d", user.TelegramID)
	menus.SendMainMenu(bot, message.Chat.ID, user)
}

// handleCheckResetCommand обрабатывает команду /check_reset
func handleCheckResetCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	log.Printf("HANDLE_CHECK_RESET_COMMAND: Выполнение команды /check_reset для TelegramID=%d", message.From.ID)

	// Проверяем, что пользователь - админ
	if message.From.ID != common.ADMIN_ID {
		log.Printf("HANDLE_CHECK_RESET_COMMAND: Пользователь TelegramID=%d не является админом", message.From.ID)
		msg := tgbotapi.NewMessage(message.Chat.ID, "🚫 Доступ запрещён")
		if _, err := bot.Send(msg); err != nil {
			log.Printf("HANDLE_CHECK_RESET_COMMAND: Ошибка отправки сообщения о запрете: %v", err)
		}
		return
	}

	// Отправляем сообщение о начале проверки
	msg := tgbotapi.NewMessage(message.Chat.ID, "🔍 Запуск проверки состояния reset...")
	if _, err := bot.Send(msg); err != nil {
		log.Printf("HANDLE_CHECK_RESET_COMMAND: Ошибка отправки сообщения: %v", err)
		return
	}

	// Запускаем проверку в отдельной горутине
	go func() {
		// Создаем временный экземпляр сервиса для ручной проверки
		checker := services.NewResetStatusChecker()

		// Выполняем проверку
		checker.CheckAndFixResetStatus()
...
// handleCheckDepletedCommand обрабатывает команду /check_depleted
func handleCheckDepletedCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	log.Printf("HANDLE_CHECK_DEPLETED_COMMAND: Выполнение команды /check_depleted для TelegramID=%d", message.From.ID)

	// Проверяем, что пользователь - админ
	if message.From.ID != common.ADMIN_ID {
		log.Printf("HANDLE_CHECK_DEPLETED_COMMAND: Пользователь TelegramID=%d не является админом", message.From.ID)
		msg := tgbotapi.NewMessage(message.Chat.ID, "🚫 Доступ запрещён")
		if _, err := bot.Send(msg); err != nil {
			log.Printf("HANDLE_CHECK_DEPLETED_COMMAND: Ошибка отправки сообщения о запрете: %v", err)
		}
		return
	}

	// Отправляем сообщение о начале проверки
	msg := tgbotapi.NewMessage(message.Chat.ID, "🔍 Запуск проверки ложных состояний 'исчерпано'...")
	if _, err := bot.Send(msg); err != nil {
		log.Printf("HANDLE_CHECK_DEPLETED_COMMAND: Ошибка отправки сообщения: %v", err)
		return
	}
	...
	go func() {
		// Выполняем проверку
		services.ForceCheckDepletedStatus()
